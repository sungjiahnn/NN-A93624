% Sung Ji Ahn 2025
% plot intravenous SO2 OEF traces A/V, export first peak and shallow peak values for
% stats 
...
function plot_SO2_traces_WT_vs_PS19()
    ...
    % Plot sO2 and OEF
    plot_sO2_and_OEF(data, resample_time);
...
function data = process_group(file_list, path, vessel_types, resample_time, time)
    ...
    for i = 1:length(file_list)
        ...
        for v = 1:length(vessel_types)
            ...
            mean_raw = mean(smoothed, 1, 'omitnan');
...
function export_peak_summary(data, time, peak1_range, peak2_range)
    ...
    vessels = {'a', 'v'};
    peak_data = {};
    header = {'Group', 'Vessel', 'MouseID', 'sO2_Peak1', 'sO2_Peak2', 'OEF_Peak1', 'OEF_Peak2'};
    ...
    for g = 1:2
        ...
        for v = 1:2
            ...
            for i = 1:size(sO2_mat, 1)
                ...
                peak_data(end+1, :) = {group, vessel, i, sO2_1, sO2_2, oef1, oef2};
            end
        end
    end
...
function export_oef_traces(data, resample_time)
    groups = {'WT', 'PS19'};
    output = {};
